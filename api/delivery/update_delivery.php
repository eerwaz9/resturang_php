...
if (
 ...
 && isset($_POST["del_note"])
		...
		if (!empty($_FILES["file"]['name']) )
	{
		...
		$del_image = $images["image"];
		$del_thumbnail = $images["thumbnail"];
	...
	if($del_image != "")
	{
		array_push($updateArray, htmlspecialchars(strip_tags($del_image)));
		array_push($updateArray, htmlspecialchars(strip_tags($del_thumbnail)));
	}
	...
	{
			$sql = "update delivery 
		set del_name=?,del_pwd=?,del_mobile=?,del_note=?
		where del_id=?";
	}
