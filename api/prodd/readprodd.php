...
if (
	...
	if(trim($txtsearch) != "")
	{
		$sql = "select * from prodd where  cat_id = ? and prod_name like ? order by prod_id desc limit $start , $end";
		$result = dbExec($sql, $selectArray);
	}
	else
	{
		$sql = "select * from prodd where  cat_id = $cat_id order by prod_id desc limit $start , $end";
		$result = dbExec($sql, []);
	}
    $arrJson = array();
